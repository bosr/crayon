...
def get_run_tags(tensorboard_url, runname, (q_plugins, q_tags, q_format, q_last)):
    ...
    for plugin, taglist in run_tags.items():
        if (plugin in q_plugins) or not q_plugins:
            ...
            requested_run_tags[plugin] = [tag for tag in taglist if tag in q_tags] if q_tags else taglist

    # get samples for each plugin & each tag in the run
    data = defaultdict(dict)
    for plugin, taglist in requested_run_tags.items():
        for tag in taglist:
            url = tensorboard_url + '/data/plugin/' + plugin + '/' + plugin + '?run=' + runname + '&tag=' + tag
    ...
    for plugin, tag_dict in data.items():
        for tag, samples in tag_dict.items():
            json_data[plugin][tag] = [{'wall_time': s[0], 'step': s[1], 'value': s[2]} for s in samples]
    return json_data
